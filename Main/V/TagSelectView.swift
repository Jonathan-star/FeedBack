...
class CBGroupAndStreamView: UIView {
    ...
    func setDataSource(contentArr : Array<Any>, titleArr : Array<String>){
        ...
        if defaultGroupSingleArr.count != titleArr.count && !defaultGroupSingleArr.isEmpty{
            assert(defaultGroupSingleArr.count == titleArr.count, "默认选择的defaultGroupSingleArr.count 要 与titleArr.count一致")
            return
        }

        tempContentArr = contentArr.count > 0 ? contentArr : tempContentArr
...
//        print(dataSourceArr)
        ...
        for (index,title) in titleArr.enumerated() {
            ...
            frameRect = setupGroupAndStream(content: contentArr[index] as! Array<Any>, titleStr: title, currFrame: frameRect, groupId: index)
        }
        //设置滚动范围
...
//        print("----\(sender/.tag)")
    ...
    public func reload(){
        ...
        setDataSource(contentArr: tempContentArr, titleArr: tempTitleArr as! Array<String>)
    }
